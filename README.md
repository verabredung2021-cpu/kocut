...
- ✅ 표준 파일(SRT/EDL/FCPXML/Premiere XML)로 출력 — Premiere·DaVinci·FCP·곰믹스 등 어디서나 import
...
## v0.9.1 integration patch — Premiere XML + preview 이식

0.9.1은 클로드 v0.6.0에서 유용했던 실무 부품만 v0.9 editorial-brain 위에 이식한 버전입니다. v0.9의 한국어 편집 정책은 유지합니다.

- Premiere용 FCP7 XML(`*.premiere.xml`, xmeml)을 추가 출력합니다. Premiere는 `.fcpxml`을 직접 import하지 못하므로 이 파일을 우선 사용하세요.
- `kocut preview` 명령을 추가했습니다. EDL을 NLE에 넣기 전에 저해상도 MP4로 컷 적용 결과를 빠르게 확인할 수 있습니다.
- `parse_edl_keep_ranges`를 복구해 원본 임베디드 시작 TC가 있는 EDL도 preview에서 보정합니다.
- `smooth_cuts`에 시작/끝 가장자리 보호를 다시 넣어, 영상 첫/끝의 짧은 실제 발화가 min-clip 필터로 사라지는 문제를 줄였습니다.
- 미사용 `librosa` 의존성을 제거했습니다. 무음 fallback은 `soundfile + numpy`만 사용합니다.

미리보기 예시:

```powershell
kocut preview "D:\path\to\video.mp4" "D:\out\video.cuts.safe.edl" `
  -o "D:\out\video.safe.preview.mp4" `
  --height 480
```
...
- Python 3.10–3.12 (**3.12 권장**). 3.13은 일부 음성/ASR 의존성 휠 호환 문제가 있을 수 있어 아직 권장하지 않습니다.
- FFmpeg (PATH에 등록)
...
- `video.fcpxml` — 컷 반영 FCPXML(beta). DaVinci Resolve/FCPX용.
- `video.premiere.xml` — Premiere Pro용 FCP7 XML(xmeml, beta). Premiere에서는 이 파일을 우선 import하세요.
...
**컷 적용하기 (Premiere XML 권장):**
1. 파일 → 가져오기 → `video.premiere.xml` 선택
2. 원본 경로가 맞으면 자동 relink에 가깝게 열립니다. 안 되면 원본 영상을 relink하세요.
3. EDL도 함께 출력되지만, Premiere에서는 `.premiere.xml`을 먼저 확인하는 것을 권장합니다.
4. 검토 후 직접 다듬기

**DaVinci Resolve** — `.fcpxml` 또는 EDL을 import하세요.
...
kocut/
  ...
  fcpxml.py        # DaVinci/FCPX용 FCPXML 출력 (beta)
  xmeml.py         # Premiere용 FCP7 XML 출력 (beta)
  preview.py       # EDL 적용 미리보기 MP4 렌더
...
- faster-whisper (MIT), Kiwi (LGPL), rapidfuzz (MIT), soundfile (BSD), numpy (BSD)
